...
const ResumeBuilder = () => {
  ...
  const handleShare = () => {
    (async () => {
      // Build a robust absolute URL to the public preview route
      const resumeUrl = `${window.location.origin}/view/${resumeId}`;

      // If resume is private, make it public first so the preview route can load it
      if (!resumeData.public) {
        try {
          await changeResumeVisibility();
        } catch (err) {
          console.error('Failed to make resume public before sharing', err);
        }
      }
      ...
      if (navigator.share) {
        ...
        window.prompt('Copy this link to share your resume:', resumeUrl);
      }
    })();
  };
